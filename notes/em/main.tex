...
\newcommand{\notforexam}{%
  \mbox{\normalfont\scriptsize\colorbox{orange!15}{%
    \color{red!70!black}\bfseries
    \enspace$\star$\enspace Not included for MATH4245 (2026) exam\enspace
  }}%
}


\title{Expectation Maximization}
%\title{Neural Network Gaussian Process and NTK for Seismology and Artificial Intelligence Workshop @ Fias}
\author{Richard Xu}
...
\Theta_{\text{MLE}} &= \argmax_{\Theta} \L(\Theta |X)\\
 &=  \argmax_{\Theta} \bigg( \sum_{i=1}^n \log \sum_{l=1}^k \alpha_l \mathcal{N} (X | \mu_l, \Sigma_l) \bigg) \quad\quad \sum_{l=1}^k \alpha_l = 1
\end{split}\end{equation}

\begin{enumerate}
\item
Unlike single mode Gaussian, we can't just take derivatives and let it equal zero easily, i.e., optimize it analytically. 

\item
In terms of optimizing its parameters: the problem is {\bf non-concave}. So traditional gradient ascend is {\bf not} suitable for it, i.e., many local maximums.

\end{enumerate}

The {\bf goal} is to find an iterative process that ensures that $\log(p(X | \Theta^{(g)}))$ remains non-decreasing for $g = 1, \dots $. 

To do so, for data $X = \{x_1, \dots x_n\}$, we introduce ``latent'' variables $Z = \{ z_1, \dots z_n\}$, each $z_i$ indicating which mixture $x_i$ belongs to. The core problem then simply boils down to a single Gaussian fitting.
...
\begin{equation}\begin{split}
f\left( (1-t) x_1 + t x_2 \right) \leq (1-t) f(x_1) + t  f(x_2)  \hspace{5 mm} t \in (0 \dots 1)
\end{split}\end{equation}
...
\begin{equation}\begin{split}
\phi \left( (1-t) x_1 + t x_2 \right) \leq (1-t) \phi (x_1) + t  \phi(x_2)  \hspace{5 mm} t \in (0 \dots 1)
\end{split}\end{equation}
...
\begin{equation}\begin{split}
\phi \left( \int_{x } f(x) p(x) \right) &\leq \int_{x }  \phi (f(x)) p(x) \implies \phi \big( \E[  f(x) ] \big) \le \E [\phi (f(x))]
\end{split}\end{equation}
...
{\bf The trick } is to assume some ``latent'' variable $Z$ to the model

For each iteration of the E-M algorithm, we perform:

\begin{equation}\begin{split}
\Theta^{(g+1)} = \argmax_{\Theta} \left( \int_z \log{(p(X,Z|\Theta))} p(Z|X,\Theta^{(g)}) \right) \d Z
\end{split}\end{equation}
...
Note the difference between variable $\Theta$ and the constant $\Theta^{(g)}$. Also note that gradient ascend is {\bf not} suitable for many E-M problems, as they can be {\bf non-concave}.


%-------------------------------------------------------------

\section[Proof of convergence: Maximization-Maximizatio]{Proof of convergence: Maximization-Maximizatio \notforexam}
...
\begin{equation}\begin{split}
\L(\Theta|X) 
&= \log p(X|\Theta) =  \log \int_Z  p(X, Z |\Theta) \\
&=\log \Big( \int_Z \frac{  p(X,Z|\Theta)}{q(Z)} q(Z)  \Big)\\
&\ge  \int_Z \log \Big( \frac{  p(X,Z|\Theta)}{q(Z)} \Big) q(Z) \\
&= \text{ELBO}(\Theta, q)
\end{split}\end{equation}
...
\L(\Theta|X) &= \int_Z \log \left(\frac{p(X,Z|\Theta)}{q(Z)} \right)q(Z) + \int_Z \log \left( \frac{ q(Z)}{p(Z|X,\Theta)} \right)q(Z)\\
...
&=\argmax_{q} \Big\{ \int_Z \log \left(\frac{p(X,Z|\Theta^{(g)})}{q(Z)} \right)q(Z) \Big\}\\
&= p(Z|X, \Theta^{(g)})\\
\implies \text{ELBO} (\Theta^{(g)}, q^* )  &= \L(\Theta^{(g)}|X)
\end{split}\end{equation}

This can be done with the realization that when fixing $\Theta = \Theta^{(g)}$, ELBO is upperbounded by $\L(\Theta^{(g)} | X)$, and maximum occurs, i.e., when $\KL(q^* \| p(Z|X, \Theta^{(g)} )) = 0$

{\bf STEP 1} is invisible to the algorithm, but needs to be considered for interpretation convergence.
...
It is obvious that, after we computed the E-M update formula, i.e., Eq.(\ref{eq M-M update formula}), we have:
...
\begin{equation}
\begin{split}
p(X|\Theta) =  \sum_{l=1}^k \alpha_l \mathcal{N} (X | \mu_l, \Sigma_l) = \prod_{i=1}^n \sum_{l=1}^k \alpha_l \mathcal{N} (X | \mu_l, \Sigma_l)
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
p(X,Z|\Theta) &= \prod_{i=1}^{n} p(x_i, z_i | \Theta) = \prod_{i=1}^{n} \underbrace{p(x_i | z_i , \Theta)}_{\mathcal{N}(\mu_{z_i}, \Sigma_{z_i} )} \underbrace{p ( z_i | \Theta)}_{\alpha_{z_i}}
= \prod_{i=1}^{n} \alpha_{z_i} \mathcal{N}(\mu_{z_i}, \Sigma_{z_i} )\\
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
p(Z|X,\Theta) &= \prod_{i=1}^{n} p(z_i | x_i , \Theta) = \prod_{i=1}^{n} \frac{\alpha_{z_i} \mathcal{N}(\mu_{z_i}, \Sigma_{z_i} )}{\sum_{l=1}^k \alpha_{l} \mathcal{N}(\mu_{l}, \Sigma_{l} )}\\
\end{split}
\end{equation}
...
\begin{equation}\begin{split}
q(\Theta, \Theta^{(g)}) &=  \int_z \log{(p(X,Z|\Theta))} p(Z|X,\Theta^{(g)}) \d z \\
&=  \int_{z_1} \dots \int_{z_n} \left( \sum_{i=1}^{n} \log p(z_i ,  x_i | \Theta) \prod_{i=1}^n p(z_i|x_i,\Theta^{(g)}) \right) \d z_1, \dots \d z_n \\
\end{split}\end{equation}
...
let $p(Y)$ be the joint pdf: $P(y_1,\dots y_n)$, also let $F(Y)$ be a linear function, where each term involves only one variable $y_i$, i.e.,  

\begin{equation}\begin{split}
F(Y) = f_1(x_1) + \dots f_n (x_n) = \sum_{i=1}^n f_i(y_i)
\end{split}\end{equation}
...
\int_{y_1} \dots \int_{y_n}  \Big(\sum_{i=1}^n f_i(y_i)\Big) P(Y) \d Y = \sum^N_i \bigg(  \int_{y_i} f_i(y_i) P_i(y_i) \d y_i \bigg)
\end{split}\end{equation}
...
\begin{equation}
\int_Y F(Y) p(Y) \d Y = \int_{y_1} \int_{y_2}\dots \int_{y_N}  \Big(   \sum_{i=1}^N f_i(y_i) \Big) p(Y) \d y_1, \dots \d y_n 
\end{equation}
...
\begin{equation}
\begin{split}
\int_Y (F(Y))P(Y) \d Y =  \int_{y_1} f_1(y_1) P_1(y_1) \d y_1 + \dots +  \int_{y_n} f_n(y_n) P_n(y_n) \d y_n
\end{split}
\end{equation}
...
\begin{equation}\begin{split}
q(\Theta, \Theta^{(g)}) &=  \int_{z_1} \dots \int_{z_n} \left( \sum_{i=1}^{n} \log p(z_i , x_i | \Theta) \prod_{i=1}^n p(z_i|x_i,\Theta^{(g)}) \right) \d z_1, \dots \d z_n \\
&= \sum_{i=1}^{n} \left(  \int_{z_i}  \log p(z_i , x_i | \Theta)  p(z_i|x_i,\Theta^{(g)}) \d z_i \right)  \hspace{5 mm} z_i \in \{1,\dots, k\} \\
&= \sum_{z_i=1}^k  \sum_{i=1}^{n}  \log p(z_i , x_i | \Theta) p(z_i|x_i,\Theta^{(g)}) \hspace{5 mm} \text{swap the summation terms}\\
&= \sum_{l=1}^k  \sum_{i=1}^{n}  \log [ \alpha_{l} \mathcal{N}(x_i | \mu_{l}, \Sigma_{l} )] p(l|x_i,\Theta^{(g)})  \hspace{5 mm} \text{substitute Gaussan and replace } z_i \rightarrow l\\
\end{split}\end{equation}
...
\end{axis}
\end{tikzpicture}
\caption{compute responsibility probabilities, red and blue each indicate its own un-normalized responsibilities}
\end{center}
\end{figure}
...
\subsection[The M-Step: maximizing $\mu, \Sigma$]{The M-Step: maximizing $\mu, \Sigma$ \notforexam} 
...
\begin{itemize}
     \item You will need some linear algebra identities to solve this. It's quite involved. For details, please refer:
     \item J. Bilmes. ``A Gentle Tutorial on the EM Algorithm and its Application to Parameter Estimation for Gaussian Mixture and Hidden Markov Models'' 
 \end{itemize}
...
\item Derivatives of Traces of inverse, fact 1
...
\begin{equation}
\begin{split}
\L \equiv \L(p(\Y| \K)) &= - \frac{D N }{2} \log(2 \pi)  - \frac{D}{2} \log | \K | - \frac{1}{2}  \tr \big( \K^{-1} \Y {\Y}^\top \big)\\
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
\implies \mathcal{S}(\mu_l, \Sigma_l^{-1})  &=   - \tr \left( \frac{\Sigma_l^{-1}}{2} \sum_{i=1}^{n}  ( x_i - \mu_l)  (x- \mu_l)^T p(l|x_i,\Theta^{(g)})   \right)   + \text{ Constant} \\
%----------------------------------------------------------
...
\text{second part of } q(\Theta, \Theta^{(g)})
&=  \sum_{l=1}^k  \sum_{i=1}^{n} \log [\mathcal{N}(x_i | \mu_{l}, \SIGMA_{l} ) ] p(l|x_i,\Theta^{(g)})\\
&=  \sum_{i=1}^{n} \sum_{l=1}^k \log \left(  \frac{1}{\sqrt {(2\pi )^{d}|{ \SIGMA_l}|}}  {\exp \bigg(-{\frac {1}{2}}( x_i - \MU_l )^\top \SIGMA^{-1}(x_i - \MU_l )\bigg)} \right) p(l|x_i,\Theta^{(g)})\\
%\implies \mathcal{S}(\mu_{\color{red} l},  \SIGMA_{\color{red} l}) 
%&=   \sum_{i=1}^{n}  -\frac{1}{2} \log ( | \SIGMA_l | ) p(l|x_i,\Theta^{(g)})  -  \sum_{i=1}^{n}  \frac{1}{2}  ( x_i - \MU_l)^T \SIGMA^{-1} (x- \MU_l)  p(l|x_i,\Theta^{(g)})\\
\end{split}
\end{equation}
...
\implies  \Y \P \Y^\top  \SIGMA_l^{-1}  &=  \tr(\P)  
\implies  \SIGMA_l^{-1}  =  \tr(\P)  (\Y \P \Y^\top)^{-1}  \\
\implies  \SIGMA_l  &=  \tr(\P)^{-1}  (\Y \P \Y^\top) 
=  \frac{(\Y \P \Y^\top)}{\tr(\P) }\\
 &=  \frac{\sum_{i=1}^{n}  ( x_i - \mu_l) (x- \mu_l)^T p(l | x_i,\Theta^{(g)})}{ \sum_{i=1}^{n} p(l | x_i,\Theta^{(g)})} \\
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
 \mathcal{S}(\mu_l, \Sigma_l^{-1}) &=   \sum_{i=1}^{n}   \left(  -\frac{1}{2} \log ( | \Sigma_l | )  -\frac{1}{2} ( x_i - \mu_l)^T \Sigma^{-1} (x- \mu_l) \right)  p(l|x_i,\Theta^{(g)})\\
\end{split}
\end{equation}
...
\begin{equation}\begin{split}
 &=    \Bigg(   \sum_{i=1}^{n}  \log ( | \Sigma_l^{-1} | )  p(l|x_i,\Theta^{(g)})  -\frac{1}{2} \text{tr} \Bigg( \Sigma^{-1} \underbrace{\sum_{i=1}^{n}  ( x_i - \mu_l) (x- \mu_l)^T  p(l|x_i,\Theta^{(g)}) }_{M_l} \Bigg) \Bigg) \\
%------------------------------------------------------------
\implies  \frac{\partial \mathcal{S}(\mu_l, \Sigma_l^{-1})}{\partial \Sigma^{-1}_l} &=     \frac{2   \sum_{i=1}^{n} \Sigma_l p(l|x_i,\Theta^{(g)}) -  \sum_{i=1}^{n} \text{diag}(\Sigma) p(l|x_i,\Theta^{(g)}) }{2}    -\frac{2M_l - \text{diag}(M_l)}{2} = 0 \\
%------------------------------------------------------------
\implies & 2 (  \sum_{i=1}^{n} \Sigma p(l | x_i,\Theta^{(g)}) - M_l)   -  \sum_{i=1}^{n} \text{diag} ( \Sigma p(l | x_i,\Theta^{(g)}) - M_l)   = 0\\
%------------------------------------------------------------
...
Maximizing $\mu, \Sigma$ means that to update $\Theta^{(g)} \rightarrow \Theta^{(g+1)}$: 
...
\begin{equation}\begin{split}
\Sigma_l^{(g+1)} = \frac{\sum_{i=1}^N [x_i - \mu_l^{(i+1)}] [ x_i - \mu_l^{(i+1)} ]^\top p (l | x_i, \Theta^{(g)})}{\sum_{i=1}^N p (l | x_i, \Theta^{(g)})} 
\end{split}\end{equation}

One can verify that when the number of component $K = 1 \implies p (l=1 | x_i, \Theta^{(g)}) = 1$, then:
...
\begin{equation}\begin{split}
\Sigma_l^{(g+1)} = \frac{1}{N} \sum_{i=1}^N [x_i - \mu_l^{(i+1)}] [ x_i - \mu_l^{(i+1)} ]^\top  
\end{split}\end{equation}
...
\subsection{To show the diagram again}
...
\begin{figure}[H]
     ...
     \begin{subfigure}[b]{0.48\textwidth}
         ...
         \caption{GMM at convergence: $\Theta^{(f)}$}
     \end{subfigure}
\end{figure}
