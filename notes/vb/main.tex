...
However, let's start with a much simpler problem first. We assume that we know part of information of  distribution $p(\x)$, the prior $p(\z)$ and the prior and the likelihood $p(\x | \z)$ (hence we also know the joint density $p(\x,\z)$). Although it appears that we also know the posterior distribution $p(\z | \x)$, as we can use Bayes rule to compute the posterior distribution, i.e.,:

\begin{equation}\begin{split}
p(\z | \x) = \frac{\overbrace{p(\x | \z)}^{\text{knows}}\; \overbrace{p(\z)}^{\text{knows}}}{\underbrace{\int_{\z'} p(\x|\z') p(\z') \d \z'}_{\text{don't know analytically}} }
...
to illustate what is a observed data $\x$ and what is a latent space $\z$, let's look at the same ``student mark'' toy example you saw in both the probabilistic graphical models (which we will restrorpsectivley visit again) and MCMC (which we will not discuss in 2024) sections:
...
This note started in 2010 when I was inspired to help people read Chapter 10 of Bishop \cite{bishop2006pattern} where I was trying to explain a few things in an oversimplified (hopefully!) way. I revamped it for the class. I also added exponential family distributions and an example on LDA when the model is fully conjugate \cite{hoffman2013stochastic}  
...
\subsection{what is Evidence Lowerbound?}

\subsection{use Jensen Inequality}
...
the above do not show what is the missing bit between $\log p(\x) - \text{ELBO}(q)$. So what is it?
...
\begin{equation}\begin{split}
\log{(p(\x))} &=   \int q_\phi(\z) \log{  \left(   \frac{p(\x,\z)}{q_\phi(\z)}  \right)  } d \text{z} -  \int q_\phi(\z) \log{  \left(   \frac{p(\z|\x)}{q_\phi(\z)}  \right)  } \d \z\\
&=   \int q_\phi(\z) \log{  \left(   \frac{p(\x,\z)}{q_\phi(\z)}  \right)  } d \text{z} + \int q_\phi(\z) \log{  \left(   \frac{q_\phi(\z)}{p(\z|\x)}  \right)  } \d \z\\
&= \text{ELBO}(q) + \KL(q\|p)
\end{split}\end{equation}



\subsubsection{name to both terms}

\begin{equation}\begin{split}
\text{ELBO}(q) &= \int q_\phi(\z) \log{  \left(   \frac{p(\x,\z)}{q_\phi(\z)}  \right)  } \d \z
= \E_{z \sim q_\phi(\z) } \big[ \left(   \frac{p(\x,\z)}{q_\phi(\z)}  \right) \big]\\
\KL(q\|p) &= \int q_\phi(\z) \log{  \left(   \frac{q_\phi(\z)}{p(\z|\x)}  \right)  } \d \z 
\end{split}\end{equation}


now we can answer the question of why we do not minimize $\KL$ term directly? The {\bf key} is that the $\KL$ term contains $p(z | x)$ and ELBO term contains $p(x|z) p(z)$! Why is this the problem? Because then one has to compute:
...
since we can choose any $q_\phi(\z)$ we'd like, and since we want $\KL(\cdot)$ to be minimized, there it's ideal to make:
...
We know that $p(\x) = \text{ELBO}(q) + \KL(q\|p) $. We consider $\text{ELBO}(q)$ is the lower bound of $p(\x)$. Minimizing $\KL (q\| p)$ is the same as maximizing the lower bound $\text{ELBO}(q)$, since the addition of the two becomes $p(\x)$
\newpage
...
By letting $q(\z) = \prod_{i=1}^M q_i(z_i)$, by fixing all the other distributions $\{ q_{i \ne j} \}$,the following is the optimal update formula for $q_i(z_i)$ which we derived in \ref{eq: ELBO meanfield}:
...
note however, the expection $\E_{\z \setminus z_j} [ \log{(p(\x,\z))} ]$ may not always be tractable. Therefore, it is not the general solution to VB. Also note that the optimality only occurs for the current component-wise ELBO, and not the global ELBO $\text{ELBO}(q)$.

\subsection{derivation in section \ref{sec: the goal}?}

Since any $q(\z)$ will work, therefore, we will choose the most simple form. Suppose let's choose $q(\z)$, such that:
...
Rearrange the expression by taking a particular $q_j(z_j)$ out of the integral. Note that unlike Part $H(q)$, we are not treating any terms to $\C$. The component-wise of $- H(q,p)$ can be written as:
...
therefore, by adding an $\exp$ to the $\E_{\z \setminus z_j} \big[ \log{(p(\x,\z))} \big] $ term, we then obtained a pseudo distribution:
...
\begin{equation}\begin{split}
{H(q)}_{j} &= - \int\limits_{z_j} q_i(z_i) \log { (q_i(z_i))} \d z_j + \C\\
&= H ( q_i(z_i) )  + \C\\
\end{split}\end{equation}

where $\C$ are the term does not involve $z_j$.
...
the key to realize is that we do not need to take derivative as one would normally do. All we need is to re-arrange the terms, and to realise it's the KL term, so we can just match the two distributions, between $\exp(\E_{\z \setminus z_j} \big[ \log{(p(\x,\z))} \big]  )$ and $q_j(z_j)$. Note that both are distributions of $z_j$
...
\begin{equation}\begin{split}\label{eq ELBO as KL}
 \text{ELBO}(q_j) &= \int\limits_{z_j} q_j(z_j) \log \left[  \frac{\exp(\E_{\z \setminus z_j} \big[ \log{(p(\x,\z))} \big]  )} { q_j(z_j) }   \right] + \C.\\
&=- \KL \Big(  q_j(z_j) \| \exp(\E_{\z \setminus z_j} [ \log{(p(\x,\z))} ]) \Big)\\
\end{split}\end{equation}

Now {\bf this is the key}: We can maximise $\text{ELBO}(q)$, by minimising the KL divergence, where we can find approximate and optimal  $q_{j}^*(z_j)$, by setting KL divergence in Eq.(\ref{eq ELBO as KL}) to zero, i.e., $\exp(\E_{\z \setminus z_j} [ \log{(p(\x,\z))} ]) =  q_j^*(z_j) $. We also assume other $\{ q_i(z_i) \}_{i \ne j}$ set is fixed.
...
therefore, the algorithm, imagine $\z = \{ z_1, z_2, z_3 \}$, and at iteration $t -1$, where we already have $q_1^{(t-1)}, q_2^{(t-1)}, q_3^{(t-1)}$

\begin{equation}\begin{split}
q^{(t)}(z_1,z_2,z_3)&: \begin{cases}
{\color{red}q_1^{(t)}(z_1)} &= \exp(\E_{z_2 \sim q_2^{(t-1)}, z_3^{(t-1)} \sim q_3^{(t-1)}} [ \log{(p(\x,z_1,z_2,z_3))} ])\\ 
...
q_1^{(t+1)}(z_1) &= \exp(\E_{z_2 \sim q_2^{(t)}, z_3^{(t)} \sim q_3^{(t)}} [ \log{(p(\x,z_1,z_2,z_3))} ])\\ 
...
But we pretended that we canot compute the posterior, and we will use Variational Inference to approximate the posterior. The following is Python code for the true posterior:
...
&= \E_{q_\tau}  \Big[ \log ( p ( \mathcal{D}| \mu, \tau ) ) +  \log p (\mu | \tau) \Big] + \C \hspace{5 mm} \text{ leave out terms do NOT contain } \mu \\
&= \E_{q_\tau}  \Big[ \underbrace{ \frac{n}{2 }  \log \left( \tau \right) - \frac{\tau}{2} \sum_{i=1}^n (x_i - \mu)^2 }_{\log ( p ( \mathcal{D}| \mu, \tau ) )} \underbrace{ -  \frac{\lambda_0 \tau}{2} (\mu - \mu_0)^2}_{\log p (\mu | \gamma) }    \Big] + \C \\
&= -  \frac{\E_{q_\tau }[ \tau] }{2}  \underbrace{\bigg[  \sum_{i=1}^n (x_i - \mu)^2  +  \lambda_0 (\mu - \mu_0)^2  \bigg]}_{\text{terms taking out of $\E_{\tau}[\cdot]$ }} + \C \\
\end{split}\end{equation}

Completing the square for the $\mu$ terms (well, you guess it, we are going to trying to make it a guassian distribution $\N(\mu; \mu^\star, \tau^\star)$). So let's find out what $\mu^\star$ and $\tau^\star$ are:
...
{\bf by the way}, from Eq.(\ref{eq log q mu}), without the $ \E_{q_\tau} \big[ \cdot \big]$, we obtained the expression for $p(\mu | \mathcal{D}, \tau)$:


\begin{equation}\begin{split}
\log ( p ( \mathcal{D}| \mu, \tau ) ) + \log p (\mu | \tau)
&=  \underbrace{  - \frac{\tau}{2} \sum_{i=1}^n (x_i - \mu)^2 }_{\log ( p ( \mathcal{D}| \mu, \tau ) )} \underbrace{ -  \frac{\lambda_0 \tau}{2} (\mu - \mu_0)^2}_{\log p (\mu | \gamma) }   + \C \\
...
\subsection{Computing $\log{ ( q_{i}^*(\tau)) } = \E_{q_\mu(\mu)}  \left[ \log{(p(\mu, \tau , \mathcal{D}))} \right]$}
...
&= \E_{q_\mu}  \Big[ \underbrace{ \frac{n}{2 }  \log \left( \tau \right) - \frac{\tau}{2} \sum_{i=1}^n (x_i - \mu)^2 }_{\log ( p ( \mathcal{D}| \mu, \tau ) )} \underbrace{ -  \frac{\lambda_0 \tau}{2} (\mu - \mu_0)^2}_{\log p (\mu | \gamma) }  \underbrace{+ (a_0-1) \log (  \tau )  -b_0 \tau}_{\log p ( \tau) }   \Big] + \C \\
\end{split}\end{equation}
...
{\bf By the way}, from Eq.(\ref{eq log q tau}), without the $ \E_{q_\mu} [ \cdot ]$, we obtained expression for $p(\tau | \mathcal{D}, \mu )$:


\begin{equation}\begin{split}
\log p(\tau | \mathcal{D}, \mu ) &= \log ( p ( \mathcal{D}| \mu, \tau ) ) +  \log p (\mu | \tau) + \log p ( \tau)  + \C \\
&= \underbrace{ \frac{n}{2 }  \log \left( \tau \right) - \frac{\tau}{2} \sum_{i=1}^n (x_i - \mu)^2 }_{\log ( p ( \mathcal{D}| \mu, \tau ) )} \underbrace{ -  \frac{\lambda_0 \tau}{2} (\mu - \mu_0)^2}_{\log p (\mu | \gamma) }  \underbrace{+ (a_0-1) \log (  \tau )  -b_0 \tau}_{\log p ( \tau) }  + \C \\
&= \Big(  \underbrace{ \frac{n}{2 } + a_0   }_{a_n}  -1 \Big) \log ( \tau ) - \tau \Big( \underbrace{ b_0  + \frac{1}{2}   \sum_{i=1}^n (x_i - \mu)^2  +  \lambda_0(\mu - \mu_0)^2 }_{b_n} \Big)+ \C \\
...
\begin{cases}
a_n &= \frac{n}{2 } + a_0\\
b_n &= b_0  + \frac{1}{2}   \sum_{i=1}^n (x_i - \mu)^2  +  \lambda_0(\mu - \mu_0)^2\\
\end{cases}
\end{split}\end{equation}
...
a {\bf part of } Python code is listed for the variational parameters update. Of coure, one needs look at the full code for the complete picture.
...
\begin{equation}
\begin{split}
p (Z |\pi) &= \prod^N_{i=1} p(z_n|\pi)\\
&= \prod^N_{i=1} \prod^K_{k=1} \pi^{z_{ik}}_k\\
\end{split}
\end{equation}

The reason for which $\left(p(z_n|\pi) = \prod^K_{k=1} \pi^{z_{ik}}_k \right)$, or $\left(p(z|\pi) = \prod^K_{k=1} \pi^{z_{k}}_k\right)$, is because in Bishop, $z$ is not represented in a scalar form, but rather in a vector of dimension $K$, which has a single element $1$, and the rest are all $0$s. For example, instead of using $p(z_n = 2 | \pi = [0.2, 0.3, 0.5]) = 0.3 $, Bishop uses $p(z_n = [0, 1, 0] | \pi = [0.2, 0.3, 0.5]) = 0.3 $. In any case, this refers to the second element of $\pi$. Therefore, a more simpler and vocal representation for $p(z|\pi)$ is just the $z^{th}$ value of $\pi$.

\subsubsection{Definition for $p(X| Z, \mu, \Lambda)$:}

\begin{equation}
\begin{split}
p(X| Z, \mu, \Lambda) &= \prod^N_{i=1} p(x_n | z_n, \mu, \Lambda)\\
&\text{In normal literatures, such as Bilmes, it is defined as:}\\
&= \prod^N_{i=1} \mathcal{N}(x_n | \mu_{z_n}, \Lambda_{z_n}^{-1})\\
&\text{However, due to the vector representation of Bishop, the above is defined as:}\\
&= \prod^N_{i=1} \prod^K_{k=1} \mathcal{N}(x_n | \mu_{k}, \Lambda_{k}^{-1})^{z_{ik}}\\
\end{split}
\end{equation}
...
\begin{split}
    ...
    &= \E_{\pi} \left[ \log \prod^N_{i=1} \prod^K_{k=1} \pi^{z_{ik}}_k  \right]  + \E_{\mu, \Lambda} \left[ \log  \prod^N_{i=1} \prod^K_{k=1} \mathcal{N}(x_n | \mu_{k}, \Lambda_{k}^{-1})^{z_{ik}}  \right] + \C \\
    &= \E_{\pi} \left[ \sum^N_{i=1} \sum^K_{k=1} \log \pi^{z_{ik}}_k  \right]  + \E_{\mu, \Lambda} \left[  \sum^N_{i=1} \sum^K_{k=1} \log \mathcal{N}(x_n | \mu_{k}, \Lambda_{k}^{-1})^{z_{ik}}  \right] + \C \\
    &\text{given that:}, ( \log a^b = b \log a):\\
    &= \E_{\pi} \left[ \sum^N_{i=1} \sum^K_{k=1} z_{ik} \log \pi_k  \right]  + \E_{\mu, \Lambda} \left[  \sum^N_{i=1} \sum^K_{k=1} z_{ik} \log \mathcal{N}(x_n | \mu_{k}, \Lambda_{k}^{-1})  \right] + \C \\
    &= \sum^N_{i=1} \sum^K_{k=1} z_{ik} \E_{\pi}  [ \log \pi_k ]  +  \sum^N_{i=1} \sum^K_{k=1} z_{ik} \E_{\mu, \Lambda} [ \log \mathcal{N}(x_n | \mu_{k}, \Lambda_{k}^{-1}) ]  + \C \\
    &\text{Taking the common term to the left}, \sum^N_{i=1} \sum^K_{k=1} z_{ik}:\\
    &= \sum^N_{i=1} \sum^K_{k=1} z_{ik} \left( \E_{\pi}  [ \log \pi_k ]  + \E_{\mu, \Lambda} [ \log \mathcal{N}(x_n | \mu_{k}, \Lambda_{k}^{-1}) ] \right) + \C \\
...
\begin{equation}
\begin{split}
    \log \rho_{ik} &= \E_{\pi}  [ \log \pi_k ]  + \E_{\mu_k, \Lambda_k} [ \log \mathcal{N}(x_n | \mu_{k}, \Lambda_{k}^{-1})] \\
    &= \E_{\pi}  [ \log \pi_k ]  + \E_{\mu_k, \Lambda_k} \left[ \log \left( \frac{1}{(2\pi)^{(d/2)} } | \Lambda_k |^{1/2} \exp \left( -\frac{1}{2} (x_n - \mu_{k})^\top \Lambda_k (x_n - \mu_{k}) \right) \right) \right] \\
    &= \E_{\pi}  [ \log \pi_k ]  + \E_{\mu_k, \Lambda_k} \left[ \log (2\pi)^{\frac{-d}{2}} + \frac{1}{2} \log |\Lambda_k| + \left( -\frac{1}{2} (x_n - \mu_{k})^\top \Lambda_k (x_n - \mu_{k}) \right) \right] \\
    &= \E_{\pi}  [ \log \pi_k ]  + \E_{\mu_k, \Lambda_k} \left[ \frac{-d}{2} \log (2\pi) + \frac{1}{2} \log |\Lambda_k| - \left( \frac{1}{2} (x_n - \mu_{k})^\top \Lambda_k (x_n - \mu_{k}) \right) \right] \\
    &= \E_{\pi}  [ \log \pi_k ]  + \frac{-d}{2} \log (2\pi) + \frac{1}{2} \E_{\Lambda_k} \left[  \log |\Lambda_k| \right] - \frac{1}{2} \E_{\mu_k, \Lambda_k} \left[   (x_n - \mu_{k})^\top \Lambda_k (x_n - \mu_{k})  \right] \\
\end{split}
\end{equation}
...
In a way,  $\rho_{ik}^{z_{ik}}$ plays the same role as  $\pi \text{ in } p(z_n|\pi)$, therefore, $\sum^K_{k=1} \pi_k = 1 \implies \sum^K_{k=1} \rho_{ik} = 1$:

\begin{equation}
\begin{split}
    q^*(Z) &= \prod_{i=1}^N q^*(z_i) =  \prod^N_{i=1} \left(\frac{1}{ \sum^K_{j=1} \rho_{nj}} \prod^K_{k=1} \rho_{ik}^{z_{ik}} \right) \\
    &=  \prod^N_{i=1} \prod^K_{k=1} \frac{\rho_{ik}^{z_{ik}}}{ \sum^K_{j=1} \rho_{nj}} 
    =  \prod^N_{i=1} \prod^K_{k=1} r_{nk}^{z_{ik}}\\
\end{split}
\end{equation}

This is a multinomial distribution, therefore, $\E [z_i = k ] = r_{ik}$
...
\begin{split}
    ...
    &\text{And since each }  ( \mu_k, \Lambda_k) \text{ are independent, therefore:} \\
    &= \E_{Z} \left[ \log  \prod^N_{i=1} \prod^K_{k=1} \mathcal{N}(x_n | \mu_{k}, \Lambda_{k}^{-1})^{z_{ik}}  \right] + \E_{Z} \left[ \log p ( Z | \pi)  \right] + \log p (\pi) + \sum^K_{k=1} \log p ( \mu_k, \Lambda_k) + \C \\
    &= \E_{Z} \left[  \sum^N_{i=1} \sum^K_{k=1} \log (z_{ik}) \mathcal{N}(x_n | \mu_{k}, \Lambda_{k}^{-1})  \right] + \E_{Z} \left[ \log p ( Z | \pi)  \right] + \log p (\pi) + \sum^K_{k=1} \log p ( \mu_k, \Lambda_k) + \C\\
     &=  \sum^K_{k=1} \sum^N_{i=1} \E_{Z}[ \log (z_{ik})] \mathcal{N}(x_n | \mu_{k}, \Lambda_{k}^{-1})  + \E_{Z} \left[ \log p ( Z | \pi)  \right] + \log p (\pi) + \sum^K_{k=1} \log p ( \mu_k, \Lambda_k) + \C\\
     &= \underbrace{ \E_{Z} \left[ \log p ( Z | \pi)  \right] + \log p (\pi)}_{\log q^*(\pi)} + \underbrace{ \sum^K_{k=1} \sum^N_{i=1} \E_{Z}[ \log (z_{ik})] \mathcal{N}(x_n | \mu_{k}, \Lambda_{k}^{-1})  +  \sum^K_{k=1} \log p ( \mu_k, \Lambda_k)}_{\log q^*(\mu,\Lambda)} + \C\\
\end{split}
\end{equation}
...
&=  \Bigg( \underbrace{ \sum_{i=1}^N r_{i,k} + \alpha_0}_{a_n} -1 \Bigg) \sum^K_{k=1} \log \pi_k  + \C
= \text{DIR} \left( \pi | a_n \right)
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
\log q^*(\mu,\Lambda) &= \sum^K_{k=1} \sum^N_{i=1} \E_{Z}[ \log (z_{ik})] \mathcal{N}(x_n | \mu_{k}, \Lambda_{k}^{-1})  +  \sum^K_{k=1} \log p ( \mu_k, \Lambda_k)\\
&\text{We only have the expression for $\E_{q^*(Z)}[Z]$, but not $\E_{q^*(Z)}[ \log (Z) ]$}:
\end{split}
\end{equation}
...
Given both the prior and likelihood are exponential family distributions and they form a conjugacy pair, then the variational inference (also mean-field approximation), i.e., $q(\z) = \prod_i q_i(z_i)$ can have the following update formula:
...
\begin{equation}
	\begin{split}
	p_{\eta_\post (\z \setminus z_j )}p( z_j \; | \; \z \setminus z_j , \x  )
	\end{split}
\end{equation}

I know the probablity form of the above seems to be very complicated, however, you will see that it is no different to how we normally expressing distributions, $p_{\theta}(\z | \x)$, where:
...
It is very important to note that we are expressing a posterior distribution $p(\cdot) $ in terms of variable $z_j$. Therefore variables $\z \setminus z_j$ is now part of the parameters of the distribution, hence we use $\eta_\post (\z \setminus z_j )$ to denote the natural parameter of the distribution $p(z_j | -)$.


We can be made much clearly by using an example: if we have a posterior distribution concerning $z_1, \dots, z_3$, and we are interested in the posterior distribution of $z_2$ given $z_1$ and $z_3$ and $\x$, i.e.,
...
\begin{enumerate}
\item What is exponential family distribution?
\item What is natural parameter?
...
Most of the distributions we are going to look at are from {\bf exponential family}. They are expressed in terms of its natural parameter $\eta$:
...
\implies   \exp( - A(\eta)) \int_{x} h (x) \exp \{ T (x)^\top \eta \} = 1\\
\implies \int_{x}  h (x) \exp \{ T (x)^\top \eta  \} = \exp(A (\eta) )
\end{split}\end{equation}
...
\N(x; \mu, \sigma^2) &= (2 \pi \sigma^2)^{-1/2} \exp^{-\frac{(x-\mu)^2}{2 \sigma^2}}\\
...
\begin{equation}\begin{split}
\N_{\text{nat}}(x, \eeta) = \exp \Big(   T(x)^\top \eeta  -  \underbrace{\left( \frac{-\eta_1^2}{4 \eta_2} - \frac{1}{2} \log(-2 \eta_2)  \right) - \frac{1}{2} \log (2 \pi) }_{A(\eeta)}\Big)\\
\end{split}\end{equation}
...
\begin{equation}\begin{split}
\N(\z; \boldsymbol{\mu}, \Sigma) &= \exp \left( - \frac{1}{2} (\x - \boldsymbol{\mu})^\top \Sigma^{-1} (\x - \boldsymbol{\mu}) - \frac{1}{2} \log | \Sigma | - \frac{d}{2} \log (2 \pi) \right)\\
\end{split}\end{equation}
...
\item
The natural parameters of conditional distribution parameters correspond to the conditional distribution $p(z_1|z_2)$ is:
	

\begin{equation}\begin{split}
\eta_1 &= \frac{ \mu}{ \sigma^2}\\
&=\frac{\mu_1 + \frac{\Sigma_{1,2}}{\Sigma_{2,2}}(z_2 - \mu_2)}{\Sigma_{1,1} - \frac{\Sigma_{1,2}}{\Sigma_{2,2}}} \\
\eta_2 &= - \frac{1}{ 2 \sigma^2}\\
&= -\frac{1}{2(\Sigma_{1,1} - \frac{\Sigma_{1,2}}{\Sigma_{2,2}})}
\end{split}\end{equation}
...
\begin{equation}\begin{split}
\eta_1 &= \E_{q(z_2)} \left[ \frac{\mu_1 + \frac{\Sigma_{1,2}}{\Sigma_{2,2}}(z_2 - \mu_2)}{\Sigma_{1,1} - \frac{\Sigma_{1,2}}{\Sigma_{2,2}}} \right] \\
&= \frac{\mu_1 + \frac{\Sigma_{1,2}}{\Sigma_{2,2}}(\E_{q(z_2)}[z_2] - \mu_2)}{\Sigma_{1,1} - \frac{\Sigma_{1,2}}{\Sigma_{2,2}}} \\
\eta_2 &= -\frac{1}{2(\Sigma_{1,1} - \frac{\Sigma_{1,2}}{\Sigma_{2,2}})}
\end{split}\end{equation}

where $\eeta = \big[ \eta_1, \eta_2 \big]$

\item
Converting these natural parameters back to the standard Gaussian parameterization, converting $\eeta$ back to $\mu^{(q)}_1$ and $\sigma^2_1$, i.e., $q(z_1) \sim \N(\mu_1^{(q)},\sigma_1^2)$. The reason to have parameter $\mu_1^{(q)}$ is so that it refers to the variation distribution $q$, not the true distribution $p$.

\begin{equation}\begin{split}
\mu^{(q)}_1 = -\frac{\eta_1}{2\eta_2} &= - \frac{1}{2} \left( \frac{\mu_1 + \frac{\Sigma_{1,2}}{\Sigma_{2,2}}(\E_{q(z_2)}[z_2] - \mu_2)}{\Sigma_{1,1} - \frac{\Sigma_{1,2}}{\Sigma_{2,2}}} \right) \times  \left( (-2) \Big( \Sigma_{1,1} - \frac{\Sigma_{1,2}}{\Sigma_{2,2}}\Big) \right) \\
&= \left( \frac{\mu_1 + \frac{\Sigma_{1,2}}{\Sigma_{2,2}}(\E_{q(z_2)}[z_2] - \mu_2)}{\Sigma_{1,1} - \frac{\Sigma_{1,2}}{\Sigma_{2,2}}} \right) \times  \left(  \Sigma_{1,1} - \frac{\Sigma_{1,2}}{\Sigma_{2,2}}\right) \\
&= \mu_1 + \frac{\Sigma_{1,2}}{\Sigma_{2,2}}(\E_{q(z_2)}[z_2] - \mu_2)\\
\end{split}\end{equation}

\begin{equation}\begin{split}
\sigma_1^2 &= -\frac{1}{2\eta_2} \\
&= -\frac{1}{2} \left(-\frac{1}{2(\Sigma_{1,1} - \frac{\Sigma_{1,2}}{\Sigma_{2,2}})}\right)^{-1} \\
&= -\frac{1}{2} \left(-2(\Sigma_{1,1} - \frac{\Sigma_{1,2}}{\Sigma_{2,2}})\right) \\
&= \Sigma_{1,1} - \frac{\Sigma_{1,2}}{\Sigma_{2,2}}
\end{split}\end{equation}
...
\begin{equation}\begin{split}
p(z_2 | z_1) &= \N\left(z_2; \mu_2 + \frac{\Sigma_{2,1}}{\Sigma_{1,1}}(z_1 - \mu_1), \Sigma_{2,2} - \frac{\Sigma_{2,1}}{\Sigma_{1,1}}\right)
\end{split}\end{equation}
...
\begin{equation}\begin{split}
\eta_1 &= \frac{\mu_2 + \frac{\Sigma_{2,1}}{\Sigma_{1,1}}(z_1 - \mu_1)}{\Sigma_{2,2} - \frac{\Sigma_{2,1}}{\Sigma_{1,1}}} \\
\eta_2 &= -\frac{1}{2(\Sigma_{2,2} - \frac{\Sigma_{2,1}}{\Sigma_{1,1}})}
\end{split}\end{equation}

Note that $\Sigma_{1,2} = \Sigma_{2,1}$ in a symmetric covariance matrix. 

\item
therefore, the update formula for $q_{\eeta}(z_2)$ is:

we can express the update formula for $q_{\eeta}(z_2)$ in terms of the covariance matrix elements:
...
\sigma_2^2 &= \Sigma_{2,2} - \frac{\Sigma_{2,1}}{\Sigma_{1,1}}
\end{split}\end{equation}
...
Note that prior and posterior are of the same type, but they are usually not the same distribution, otherwise, the likelihood $p(\x | \theta)$ is not useful at all!

Using exponential family distribution representation, when conjugacy is achieved, it means the prior and posterior have the {\bf same} sufficient statistics $T(\theta)$ and $h(\theta)$, but different natural parameter, i.e., $\eta_{\text{post}}$ and $\eta_{\text{prior}}$, and different log-normalizer for both.
...
\subsection{What is the criteria for likelihood to pair up with conjugate prior? \optional}
...
p(z_{n,j} | x_{n}, z_{n,-j},\bbeta ) &= h(z_{n,j}) \exp\left\{ T (z_{n,j}) \eta_{z_{n,j}}(x_n, z_{n,-j},\bbeta)  - A_l \big(\eta_{z_{n,j}}(x_n, z_{n,-j},\bbeta)\big) \right\}\\
\end{split}\end{equation}
...
Let's work through a concrete example of posterior $p (\bbeta | x_n, z_n)$, instead of writing $\eeta_\beta$, we write $\bbeta$ directly:
...
note that $\aalpha_1$ has the same dimensionality to $\bbeta$, and $\alpha_2$ is a scalar quantity. Then the prior itself can be written as:
...
notice the posterior ``inherited'' $h(\bbeta)$ from the prior, so we only need to making sure that the $T(\bbeta)$ are the same for both prior and posterior.
...
\begin{equation}\begin{split}
p(\x, \z | \beta ) &= \prod_{n=1}^{N} \Big\{ h(x_n,z_n) \exp\{ \bbeta^\top T ( x_n, z_n) - A_l (\bbeta) \} \Big\} \\
...
When we use the expression and use $\eta_\post (\x,\z,\aalpha) $ instead:
...
\implies \exp \big( A_\post (\eta_\post (\x, \z, \aalpha)) \big) &= \int_{\bbeta} h (\bbeta) \exp \big\{ \eta_\post (\x,\z,\aalpha)^\top T(\beta) \big\} \d \bbeta
\end{split}\end{equation}
...
suppose data $x_i$ come from unit variance Gaussian. Compare with Section (\ref{sec Gaussian: Natural Parameter Representation}), we saved one parameter:
...
&= h (\beta) \exp \Big\{   \begin{bmatrix}  \alpha_1 \\ \alpha_2\end{bmatrix}^\top \; \begin{bmatrix}  \beta   \\   {\color{red}-\frac{\beta_2}{2}} \end{bmatrix}  - A_g (\alpha) \Big\}
\end{split}\end{equation}
...
We can arrange Eq.(\ref{eq conjugate prior for variance one Gaussian}) to look like, but with parameter $\begin{bmatrix}\alpha_1  & -\frac{\alpha_2}{2}  \end{bmatrix}^\top$:


\begin{equation}\begin{split}
p (\bbeta | \alpha) &= h (\bbeta) \exp \left\{  
...
Suppose the likelihood is an exponential family distribution. Every exponential family has a conjugate prior in theory. The natural parameter $\aalpha = \begin{bmatrix} \aalpha_1 & \alpha_2 \end{bmatrix}^\top$ has dimension dim$(\bbeta)$ + 1. The sufficient statistics of the prior are $\begin{bmatrix} \bbeta & -A_l(\beta)\end{bmatrix}^\top$
...
&\implies \int_{\beta}   h (\beta) \exp \big\{ \lambda^\top T(\beta) - A_g (\lambda) \big\}  \left(   T(\beta) -  \DLambda A_g (\lambda) \right)  = 0\\
&\implies \underbrace{\int_{\beta}  h (\beta) \exp \big\{ \lambda^\top T(\beta) - A_g (\lambda) \big\} T(\beta) }_{\E_{q(\beta)}[ T(\beta)]}  -
\underbrace{\int_{\beta}  h (\beta) \exp \big\{ \lambda^\top T(\beta) - A_g (\lambda) \big\}}_{=1}  \DLambda A_g (\lambda)  = 0\\
...
\begin{equation}\begin{split}
\text{ELBO}( \lambda)
&= \E_{q(\beta, \z)} [\log p (\beta | \x, \z, \alpha )] + \E_{q(\beta, \z)} [\log p (\x, \z)] - \E_{q(\beta, \z)} [\log q(\beta)] \\
...
in words, when we try to update $\lambda$ for $q(\beta | \lambda)$, it find the corresponding posterior $p (\beta | \x, \z, \alpha )$, and its natural parameter $\eta_\post (\x, \z,\alpha)$, then computes the expectation with all the $q(\cdot)$ that its natural parameter has random variable for.
...
In a very similar fashion to $\L(\lambda)$, we can prove:
...
\hspace{10 mm} $w_{dn} \sim \text{Mult} (\beta_{z_{dn}})$
...
apply the update formula, in which we need the natural parameter for $p(z_{d,n} | \ttheta_d, \bbeta_{1:K}, w_{d,n} )$ in the exception:
...
\because \sum_{k=1}^K \gamma_{d,k}  \text{ has same value irrespective of } k\\
\end{split}\end{equation}
...
p( \ttheta_d | \z_{d} ) &= p(\ttheta_d | \aalpha) \prod_{n=1}^N  p(z_{d,n} | \ttheta_d)
...
let's use $q(\eta(\ggamma_d)) = \text{Dir}((\eta(\ggamma_d))$ to approximate $p( \ttheta_d | \z_{d} ) $ (another Dirichlet distribution), we apply for the exponential family update formula Eq.(\ref{eq exponential family update}):
...
&=  \sum_{n=1}^N  \pphi_{d,n}^k
\end{split}\end{equation}

continue with Eq.(\ref{eq update for phi d n}), we have:

\begin{equation}\begin{split}
\eta(\ggamma_d) &= \begin{bmatrix}\alpha_1 - 1 + \sum_{n=1}^N  \pphi_{d,n}^k  & \dots &  \alpha_K - 1 + \sum_{n=1}^N  \pphi_{d,n}^k \end{bmatrix} \\
%&= \begin{bmatrix}(\alpha_1 - 1 + \sum_{n=1}^N \1(z_{d,n} =  1 ) \phi^1_{d,n} )  & \dots &  (\alpha_K - 1 + \sum_{n=1}^N  \1(z_{d,n} = K ) \phi^K_{d,n}) \end{bmatrix} \\
\end{split}\end{equation}


to obtain the Dirichlet distribution parameter $\ggamma_d$, luckily, for Dirichlet distribution, the natural and ``normal'' parameter are the same.
...
\begin{equation}\begin{split}
p( \bbeta_{k} | \z, \w ) &= p(\bbeta_k | \xi) \prod_{d=1}^D \prod_{n=1}^N  p \left(w_{d,n} | \bbeta_{k}\right)^{ \1(z_{d,n}= k)}
= \text{Dir} (\eta) \times  \prod_{d=1}^D \prod_{n=1}^N \beta_{k,w_{d,n}}^{\1(z_{d,n}= k) } \\
&\propto \exp\bigg( \underbrace{\left(   \xi - 1 +  \sum_{d=1}^D \sum_{n=1}^N  w_{d,n}  \1(z_{d,n}= k)    \right)}_{\eta_l \left(\eta,  Z, W \right)}  \times \underbrace{\log(\bbeta_k)}_{T(\bbeta_{k})}\bigg)\\
...
let's use $q(\eta(\llambda_k)) = \text{Dir}((\eta(\llambda_k))$ to approximate $p( \bbeta_{k} | \z, \w )$ (another Dirichlet distribution), we apply for the exponential family update formula Eq.(\ref{eq exponential family update}):
...
&= \eta -1  +  \sum_{d=1}^D \sum_{n=1}^N  w_{d,n} \phi^k_{d,n}  \\
\end{split}\end{equation}
...
\begin{equation}
\begin{split}
\implies q(Z, \theta_1\dots \ttheta_d, \beta_1 \dots \bbeta_k) 
&= \left( \prod_{d=1}^{d=D} \prod_{n=1}^N q (z_{d,n} | \pphi_{d,n})\right) \prod_{d=1}^{D} q (\ttheta_d | \ggamma_d) \prod_{k=1}^K  q (\theta _k| \llambda_k)\\
\text{ now change to:} &=  \underbrace{\left( \prod_{d=1}^{d=D} \prod_{n=1}^N q (z_{d,n} | \pphi_{d,n})\right)}_{q(Z)}  q({ \Theta}, { \beta} | Z)\\
\end{split}
\end{equation}
...
\begin{split}
	...
	&= \E_{q (Z)} \left(  \underbrace{ \E_{ q( U | Z) } \left( [\log p (Z, U )] -  [\log q( U | Z)] \right)}_{\L(q( U | Z)) }  \right)-  \E_{q(Z)} [\log q(Z)] \\
\end{split}
\end{equation}
...
\begin{equation}\begin{split}
	...
	&=  \argmax_{q( U | Z) } \left[ \E_{q (Z)} \left(  \underbrace{ \E_{ q( U | Z) } \left( [\log p_X (Z, U )] -  [\log q( U | Z)] \right)}_{\L(q( U | Z)) }  \right)-  \E_{q(Z)} [\log q(Z)] \right] \\
		&= \E_{q (Z)} \left(  \underbrace{ \argmax_{q( U | Z) } \left[  \E_{ q( U | Z) } \left( [\log p (Z, U )] -  [\log q( U | Z)] \right) \right]}_{ }  \right) -  \E_{q(Z)} [\log q(Z)] \\
		&= \E_{q (Z)} [  \underbrace{p(Z)} ]-  \E_{q(Z)} [\log q(Z)]  \\
\end{split}\end{equation}

\begin{equation}\begin{split}
 \argmax_{q( U | Z) } \left[  \E_{ q( U | Z) } \left( [\log p (Z, U )] -  [\log q( U | Z)] \right) \right] = p(Z) 
\end{split}\end{equation}


maximum occur when $q ( U | Z ) = p( U | Z) \implies \KL  \left( q( U | Z)  \| p(U | Z) \right) = 0$    
